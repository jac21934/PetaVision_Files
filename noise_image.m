...
IdentString = [ BaseString  "IdentLayer.pvp"];
...
[identData, identHdr] = readpvpfile(IdentString);
...
# Ident Picture
IdentMax = max(identData{pictureIndex}.values(:));
IdentMin = min(identData{pictureIndex}.values(:));
IdentImage = (identData{pictureIndex}.values .- IdentMin);
IdentImage = IdentImage./max(IdentImage(:));
IdentImage = (permute(IdentImage, [2,1,3]));
...
concatenated = imresize(horzcat(InputImage,NoiseImage,ErrorImage, ReconImage, IdentImage), 5);
